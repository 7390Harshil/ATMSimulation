...
public class signupThree extends JFrame implements ActionListener
{
	...
	String receiveFormNo;
	
	signupThree(String receivedFormNo)
	{
		
		this.receiveFormNo = receivedFormNo;
		setLayout(null); 
		setLayout(null);
		...
		add(card);
		JLabel number = new JLabel("XXXX-XXXX-XXXX-0502");
		number.setFont(new Font("Raleway" , Font.BOLD , 20));
		number.setBounds(330 , 278 , 400 , 20);
		add(number);
		JLabel cdetail = new JLabel("Your 16 digit Card Number");
		cdetail.setFont(new Font("Raleway" , Font.BOLD , 12));
		cdetail.setBounds(100 , 298 , 400 , 20);
		add(cdetail);
		...
		c7 = new JCheckBox("I herby declare that the above provided details are correct to the best of my knowledge.");
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource() == submit)
		{
			...
			String cardNumber = "" + Math.abs((random.nextLong() % 900000001L) + 5040936000000000L);
			...
			try
			{
				if(!c7.isSelected())
				{
					JOptionPane.showMessageDialog(null , "You have to check the declaration");
				}
				else
				{
					Connect con = new Connect();
					String query = "INSERT INTO signupThree (formno, account_type, card_number, pin_number, facility) VALUES "
					        + "('" + receiveFormNo + "', '" + account + "', '" + cardNumber + "', '" + pinNumber + "', '" + facility + "')";
					con.st.executeUpdate(query);
					
					String query2 = "INSERT INTO login (formno, card_number, pin_number) VALUES "
					        + "('" + receiveFormNo + "', '" + cardNumber + "', '" + pinNumber + "')";
